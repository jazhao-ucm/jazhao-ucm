% Strategic Management - Week 14 Master Vocabulary Bank
...
\pagestyle{fancy}
\fancyhf{}
\fancyfoot[L]{\scriptsize Strategic Management | Week 14 | Master Vocabulary}
...
{\small\color{textdark} Week 14 consolidation sheet (Units 0--6 + finals Cornell blocks + case language)}
\end{center}

\colorbox{pinksoft}{\parbox{\dimexpr\linewidth-2\fboxsep}{\small
\textbf{Source scope:} terms consolidated from `strategic-management/` materials and `strategic-management-final-cornell-master-notes-wed-thu-spring2026.tex` (Spring 2026 Wed/Thu finals).}}
...
\section*{Week 14 Finals Supplement (Lectures \& Cases)}
\begin{itemize}
\vocab{Beta ($\beta$)}{Stock return sensitivity to the market; higher $\beta$ implies higher systematic volatility versus the index.}
\vocab{Control premium}{Price paid above market value to gain control in M\&A (lecture band often 20--50\%).}
\vocab{Economic Value Added (EVA)}{Residual performance measure: operating profit after charging the full cost of capital.}
\vocab{Market capitalization}{Total equity value: share price times shares outstanding.}
\vocab{Greenwashing}{Deceptive environmental claims that overstate real sustainability performance.}
\vocab{Stock options}{Right to buy company stock at a preset strike price; governance tool to align managers with shareholders.}
\vocab{Asymmetric information}{One party (often managers) holds superior private information versus owners or outsiders.}
\vocab{Tacit knowledge}{Hard-to-codify know-how held in people's heads; walks away with turnover.}
\vocab{Causal ambiguity}{Rivals cannot pin down which internal factors cause success, so imitation fails.}
\vocab{Social complexity}{Advantage rooted in trust, culture, and relationship networks that are costly to replicate.}
\vocab{Lean production / Toyota Production System}{Systematic waste elimination and flow efficiency in operations.}
\vocab{Just-in-time (JIT)}{Coordination that minimizes inventory by producing/delivering closer to actual demand.}
\vocab{Intellectual property (IP)}{Legal rights protecting innovations and brands (patents, copyrights, trademarks).}
\vocab{Minimum efficient scale (MES)}{Smallest scale at which a firm can approach competitive unit costs; creates barrier logic when high.}
\vocab{Experience curve}{Efficiency and cost advantages accumulated through cumulative production and learning over time.}
\vocab{Perceived value}{Buyers' overall judgment of benefits relative to price (distinct from the firm's internal cost).}
\vocab{Hybrid strategy}{Concurrent cost discipline and meaningful differentiation (e.g.\ Zara-style or Toyota-style claims require integrated operating proof).}
\vocab{Ansoff matrix}{Growth paths: penetration, market development, product development, diversification.}
\vocab{Market penetration}{Same product, same market---grow share or usage intensity.}
\vocab{Market development}{Same product, new geographies or segments.}
\vocab{Product development}{New products for existing customers/markets.}
\vocab{Asset stripping}{Acquire a firm and sell off parts for cash; common in some unrelated or financial plays.}
\vocab{Synergy}{Combined value exceeds the sum of stand-alone parts (``one plus one equals three'' shorthand).}
\vocab{Low-cost carrier (LCC)}{Airline model emphasizing no-frills operations and unit-cost minimization (clock positions 1--2).}
\vocab{Full-service carrier}{Differentiated airline proposition (lounges, loyalty, service) supporting a price premium (clock $\sim$4).}
\vocab{Backward integration}{Vertical integration upstream into suppliers or inputs.}
\vocab{Forward integration}{Vertical integration downstream into distribution or direct customer access.}
\vocab{Knowledge spillovers}{Ideas diffusing across people and firms; amplified in geographic clusters.}
\vocab{Labor pooling}{Thick regional market for specialized talent reducing search and switching costs for firms (clusters).}
\vocab{Prospection / scenarios}{Planning under irreducible uncertainty using alternative futures rather than a single long forecast.}
\vocab{Grant model (four pillars)}{Long-term goals, environment understanding, honest resource appraisal, effective implementation.}
\vocab{Lobbying (industrial)}{Political activity shaping rules, tariffs, and entry barriers that alter competitive ``rules of the game.''}
\vocab{Cluster anchor institution}{Universities and public R\&D that seed talent and innovation (e.g.\ Stanford/Berkeley, MIT/Harvard frames in lecture).}
\vocab{Mercadona (illustration)}{Spanish supermarket chain used as hybrid high perceived-value / disciplined price example on the strategy clock.}
\vocab{Third place (Starbucks)}{Between home and work: atmosphere, community, and habit formation supporting differentiation and clustering.}
\vocab{Store clustering}{Dense geographic rollout to build visibility, routine, and operational learning.}
\vocab{Monopoly trap (Telefónica illustration)}{High price and weak perceived value sustainable only while competition is blocked; liberalization forces real cost/differentiation credibility.}
\end{itemize}
