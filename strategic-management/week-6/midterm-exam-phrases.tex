...
\section*{\color{eublue}17. Emergency checklist (when your brain freezes)}
\begin{itemize}
\item \textbf{Step 1:} Write one opener: \emph{``This question is essentially about \textbf{[industry attractiveness / internal vs external / CA in unattractive industry / intangibles]}.''}
\item \textbf{Step 2:} Pick one framework and say it: \emph{``From a \textbf{Porter Five Forces} perspective\ldots''} or \emph{``Using the \textbf{resource-based view (RBV)}\ldots''}
\item \textbf{Step 3:} Choose \textbf{2--3 strongest points} (e.g.\ supplier power + substitutes; or one VRIO resource + one weakness) and run the master pattern on each.
\item \textbf{Step 4:} Close with one line: \emph{``In summary, these elements reduce/increase \textbf{profit potential} and imply that the firm/industry is \textbf{(un)attractive} and should \textbf{[differentiate / consolidate / invest in VRIO resources]}.''}
\end{itemize}
