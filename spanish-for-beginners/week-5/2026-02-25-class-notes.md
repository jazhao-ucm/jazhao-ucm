# Spanish for Beginners - Class Notes
## February 25–26, 2026 - Week 5

**Focus:** Unit 3 Listening, vocabulario; exam preparation.

---

I. Listening Datos Personales
   A. Mark 5 Correct
      1. Nombre
      2. Lugar de origen
      3. Nacionalidad
      4. Edad
      5. Profesión
   B. Do Not Mark (Unless Said)
      1. Lengua materna
      2. Aficiones
      3. Lenguas que aprende

II. Listening Una Fiesta
   A. Format
      1. True/False
      2. Listen carefully
   B. Examples from Practice
      1. Ana: not from Argentina; not cocinera
      2. Elvira: colombiana
      3. Stephanie: not Dutch
      4. José Antonio: profesor de español

III. Vocabulario Útil
   A. Lugares
      1. Madrid
      2. Valencia
      3. Bogotá
      4. Buenos Aires
      5. San José
      6. Quito
      7. Tegucigalpa
      8. Asunción
      9. Bruselas
      10. Oporto
      11. Milán
   B. Datos
      1. Nombre
      2. Apellido
      3. Edad
      4. Profesión
      5. Nacionalidad
      6. Lugar de origen
   C. Conectores
      1. Y (and)
      2. Pero (but)
      3. Porque (because)
      4. De (from/of)

IV. Exam Tips
   A. Agreement
      1. Nacionalidad & profesión match person (m/f)
   B. Spelling
      1. 6 = seis
      2. Quién = qu (not just q)
   C. Listening Strategy
      1. Focus on name
      2. Focus on origin
      3. Focus on nationality
      4. Focus on profession
      5. Focus on age
   D. Gapfill Strategy
      1. Read full sentence for context
      2. Check gender (mi amiga → -a)
   E. Por/Para Rule
      1. Infinitive after? → para
      2. Noun after? → por
      3. Clause with verb? → porque

X. 30-Second Cold Call Version.
A. 3-Line Speaking Frame.
   1. TOPIC: STATE THE CORE QUESTION IN ONE LINE.
   2. LOGIC: GIVE 2 DRIVERS/FACTORS THAT EXPLAIN THE ANSWER.
   3. ACTION: END WITH ONE DECISION OR IMPLICATION.
B. Delivery Rule.
   1. 25-30 SECONDS MAX.
   2. NO BACKGROUND STORY; GO STRAIGHT TO CLAIM + EVIDENCE + CLOSE.

DI. Oral Answer Template (60-90 Seconds, Class Use).
A. Opening.
   1. STATE THE CORE QUESTION OR TOPIC IN ONE LINE.
   2. STATE YOUR MAIN CLAIM/THESIS IN ONE LINE.
B. Logic.
   1. GIVE 2-3 KEY FACTORS OR DRIVERS.
   2. LINK EACH FACTOR TO THE CLAIM (CAUSE -> EFFECT).
C. Action Or Implication.
   1. END WITH ONE DECISION, RECOMMENDATION, OR CONSEQUENCE.
   2. CLOSE CLEARLY WITHOUT ADDING NEW IDEAS.
D. Delivery Rule.
   1. 60-90 SECONDS.
   2. CLAIM -> EVIDENCE -> CLOSE.
